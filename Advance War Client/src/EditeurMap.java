...
public class EditeurMap extends JFrame implements MouseListener,ActionListener {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for(int i=0;i<panel_1.tabInitialise.length;i++){
					for(int j=0;j<panel_1.tabInitialise[0].length;j++){
						// on recup�re les valeures du tableau dans le panel que l'on paint
						// on recup�re le num d'appartenance au debut de chaque nom
						String sAppartient = panel_1.tabInitialise[i][j].split("_")[0];
						// on recup�re le nom du type de case en fin de mot
						String sTypeCase = panel_1.tabInitialise[i][j].split("_")[1].toUpperCase();
						// si le nom coontenait un underscore on recolle la fin du mot coup� par le split
						...
						if(Integer.parseInt(sAppartient) != 0){
							// si le nom est celui de batiment qg ou batiment base on met a true la cellule bTabJoueur avec -1 car il y a un de d�callage (notre tableau ne prend pas en compte le joueur neutre alors que les valeures r�cup�r�es si
				...
				// on regarde si toutes les donn�es sont bien a true
				for(int i=0;i<iNbJ;i++){
				...
				if(manqueBase==true){
					...
					if(nomVide()){// On verrifie si le fichier n'est pas vide
						...
						if(fileExistant()){
							switch(JOptionPane.showConfirmDialog(null, "Cette map existe deja, continuer quand m�me ?","Attention !", JOptionPane.WARNING_MESSAGE)  )
							{
								case 0 ://OKay
									break;
								case 2://Annuler
	...
	public void genererLeFichierXml(){
		...
		JOptionPane.showMessageDialog(null, "Cr�ation de la map \""+this.textField_1.getText()+"\" r�ussie !","Map", JOptionPane.INFORMATION_MESSAGE);
		/////////////////////////////////////////////////////////////////
	...
	public boolean fileExistant(){
		...
		for(i=0;i<listefichiers.length;i++){ // pour toute cette liste on verrifie les egalit�es
